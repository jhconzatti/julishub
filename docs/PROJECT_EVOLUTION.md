...
| JH-014 | P1 | API/Integration | Exchange rates usam contrato all-or-nothing, permitindo que uma falha de provider ou par torne todos os dados indisponíveis. | R1 aceitou dados parciais no contrato API/frontend, mas o smoke de produção continuou em 503 porque a AwesomeAPI ainda recebia uma requisição agregada. R2 coleta pares fiat individualmente e preserva os pares válidos, sem fabricar dados. | Resolved locally after R2 — production validation pending |
